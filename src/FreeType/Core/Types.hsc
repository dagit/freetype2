...
foreign import capi unsafe "freetype/fttypes.h FT_MAKE_TAG"
  ft_make_tag :: #{type char} -> #{type char} -> #{type char} -> #{type char} -> FT_Tag

pattern FT_MAKE_TAG :: #{type char} -> #{type char} -> #{type char} -> #{type char} -> FT_Tag
