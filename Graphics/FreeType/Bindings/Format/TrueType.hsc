...
module Graphics.FreeType.Bindings.Format.TrueType where
...
data TT_Header = TT_Header
                   { h_Table_Version       :: FT_Fixed
                   , h_Font_Revision       :: FT_Fixed
                   , h_CheckSum_Adjust     :: FT_Long
                   , h_Magic_Number        :: FT_Long
                   , h_Flags               :: FT_UShort
                   , h_Units_Per_EM        :: FT_UShort
                   , h_Created             :: Ptr FT_ULong
                   , h_Modified            :: Ptr FT_ULong
                   , hXMin                 :: FT_Short
                   , hYMin                 :: FT_Short
                   , hXMax                 :: FT_Short
                   , hYMax                 :: FT_Short
                   , h_Mac_Style           :: FT_UShort
                   , h_Lowest_Rec_PPEM     :: FT_UShort
                   , h_Font_Direction      :: FT_Short
                   , h_Index_To_Loc_Format :: FT_Short
                   , h_Glyph_Data_Format   :: FT_Short
                   }
...
instance Storable TT_Header where
  ...
  poke ptr val = do
    #{poke struct TT_Header_, Table_Version      } ptr $ h_Table_Version       val
    #{poke struct TT_Header_, Font_Revision      } ptr $ h_Font_Revision       val
    #{poke struct TT_Header_, CheckSum_Adjust    } ptr $ h_CheckSum_Adjust     val
    #{poke struct TT_Header_, Magic_Number       } ptr $ h_Magic_Number        val
    #{poke struct TT_Header_, Flags              } ptr $ h_Flags               val
    #{poke struct TT_Header_, Units_Per_EM       } ptr $ h_Units_Per_EM        val
    #{poke struct TT_Header_, Created            } ptr $ h_Created             val
    #{poke struct TT_Header_, Modified           } ptr $ h_Modified            val
    #{poke struct TT_Header_, xMin               } ptr $ hXMin                 val
    #{poke struct TT_Header_, yMin               } ptr $ hYMin                 val
    #{poke struct TT_Header_, xMax               } ptr $ hXMax                 val
    #{poke struct TT_Header_, yMax               } ptr $ hYMax                 val
    #{poke struct TT_Header_, Mac_Style          } ptr $ h_Mac_Style           val
    #{poke struct TT_Header_, Lowest_Rec_PPEM    } ptr $ h_Lowest_Rec_PPEM     val
    #{poke struct TT_Header_, Font_Direction     } ptr $ h_Font_Direction      val
    #{poke struct TT_Header_, Index_To_Loc_Format} ptr $ h_Index_To_Loc_Format val
    #{poke struct TT_Header_, Glyph_Data_Format  } ptr $ h_Glyph_Data_Format   val



data TT_HoriHeader = TT_HoriHeader
                       { hh_Version               :: FT_Fixed
                       , hh_Ascender              :: FT_Short
                       , hh_Descender             :: FT_Short
                       , hh_Line_Gap              :: FT_Short
...
instance Storable TT_HoriHeader where
  ...
  poke ptr val = do
    #{poke struct TT_HoriHeader_, Version               } ptr $ hh_Version               val
    #{poke struct TT_HoriHeader_, Ascender              } ptr $ hh_Ascender              val
    #{poke struct TT_HoriHeader_, Descender             } ptr $ hh_Descender             val
    #{poke struct TT_HoriHeader_, Line_Gap              } ptr $ hh_Line_Gap              val
    #{poke struct TT_HoriHeader_, advance_Width_Max     } ptr $ hhAdvance_Width_Max      val
    #{poke struct TT_HoriHeader_, min_Left_Side_Bearing } ptr $ hhMin_Left_Side_Bearing  val
    #{poke struct TT_HoriHeader_, min_Right_Side_Bearing} ptr $ hhMin_Right_Side_Bearing val
    #{poke struct TT_HoriHeader_, xMax_Extent           } ptr $ hhXMax_Extent            val
    #{poke struct TT_HoriHeader_, caret_Slope_Rise      } ptr $ hhCaret_Slope_Rise       val
    #{poke struct TT_HoriHeader_, caret_Slope_Run       } ptr $ hhCaret_Slope_Run        val
    #{poke struct TT_HoriHeader_, caret_Offset          } ptr $ hhCaret_Offset           val
    #{poke struct TT_HoriHeader_, Reserved              } ptr $ hh_Reserved              val
    #{poke struct TT_HoriHeader_, metric_Data_Format    } ptr $ hhMetric_Data_Format     val
    #{poke struct TT_HoriHeader_, number_Of_HMetrics    } ptr $ hhNumber_Of_HMetrics     val
    #{poke struct TT_HoriHeader_, long_metrics          } ptr $ hhLong_metrics           val
    #{poke struct TT_HoriHeader_, short_metrics         } ptr $ hhShort_metrics          val



data TT_VertHeader = TT_VertHeader
                       { vh_Version                :: FT_Fixed
                       , vh_Ascender               :: FT_Short
                       , vh_Descender              :: FT_Short
                       , vh_Line_Gap               :: FT_Short
...
instance Storable TT_VertHeader where
  ...
  poke ptr val = do
    #{poke struct TT_VertHeader_, Version                } ptr $ vh_Version                val
    #{poke struct TT_VertHeader_, Ascender               } ptr $ vh_Ascender               val
    #{poke struct TT_VertHeader_, Descender              } ptr $ vh_Descender              val
    #{poke struct TT_VertHeader_, Line_Gap               } ptr $ vh_Line_Gap               val
    #{poke struct TT_VertHeader_, advance_Height_Max     } ptr $ vhAdvance_Height_Max      val
    #{poke struct TT_VertHeader_, min_Top_Side_Bearing   } ptr $ vhMin_Top_Side_Bearing    val
    #{poke struct TT_VertHeader_, min_Bottom_Side_Bearing} ptr $ vhMin_Bottom_Side_Bearing val
    #{poke struct TT_VertHeader_, yMax_Extent            } ptr $ vhYMax_Extent             val
    #{poke struct TT_VertHeader_, caret_Slope_Rise       } ptr $ vhCaret_Slope_Rise        val
    #{poke struct TT_VertHeader_, caret_Slope_Run        } ptr $ vhCaret_Slope_Run         val
    #{poke struct TT_VertHeader_, caret_Offset           } ptr $ vhCaret_Offset            val
    #{poke struct TT_VertHeader_, Reserved               } ptr $ vh_Reserved               val
    #{poke struct TT_VertHeader_, metric_Data_Format     } ptr $ vhMetric_Data_Format      val
    #{poke struct TT_VertHeader_, number_Of_VMetrics     } ptr $ vhNumber_Of_VMetrics      val
    #{poke struct TT_VertHeader_, long_metrics           } ptr $ vhLong_metrics            val
    #{poke struct TT_VertHeader_, short_metrics          } ptr $ vhShort_metrics           val
...
instance Storable TT_OS2 where
  ...
  poke ptr val = do
    #{poke struct TT_OS2_, version                } ptr $ osVersion                 val
    #{poke struct TT_OS2_, xAvgCharWidth          } ptr $ osXAvgCharWidth           val
    #{poke struct TT_OS2_, usWeightClass          } ptr $ osUsWeightClass           val
    #{poke struct TT_OS2_, usWidthClass           } ptr $ osUsWidthClass            val
    #{poke struct TT_OS2_, fsType                 } ptr $ osFsType                  val
    #{poke struct TT_OS2_, ySubscriptXSize        } ptr $ osYSubscriptXSize         val
    #{poke struct TT_OS2_, ySubscriptYSize        } ptr $ osYSubscriptYSize         val
    #{poke struct TT_OS2_, ySubscriptXOffset      } ptr $ osYSubscriptXOffset       val
    #{poke struct TT_OS2_, ySubscriptYOffset      } ptr $ osYSubscriptYOffset       val
    #{poke struct TT_OS2_, ySuperscriptXSize      } ptr $ osYSuperscriptXSize       val
    #{poke struct TT_OS2_, ySuperscriptYSize      } ptr $ osYSuperscriptYSize       val
    #{poke struct TT_OS2_, ySuperscriptXOffset    } ptr $ osYSuperscriptXOffset     val
    #{poke struct TT_OS2_, ySuperscriptYOffset    } ptr $ osYSuperscriptYOffset     val
    #{poke struct TT_OS2_, yStrikeoutSize         } ptr $ osYStrikeoutSize          val
    #{poke struct TT_OS2_, yStrikeoutPosition     } ptr $ osYStrikeoutPosition      val
    #{poke struct TT_OS2_, sFamilyClass           } ptr $ osSFamilyClass            val
    #{poke struct TT_OS2_, panose                 } ptr $ osPanose                  val
    #{poke struct TT_OS2_, ulUnicodeRange1        } ptr $ osUlUnicodeRange1         val
    #{poke struct TT_OS2_, ulUnicodeRange2        } ptr $ osUlUnicodeRange2         val
    #{poke struct TT_OS2_, ulUnicodeRange3        } ptr $ osUlUnicodeRange3         val
    #{poke struct TT_OS2_, ulUnicodeRange4        } ptr $ osUlUnicodeRange4         val
    #{poke struct TT_OS2_, achVendID              } ptr $ osAchVendID               val
    #{poke struct TT_OS2_, fsSelection            } ptr $ osFsSelection             val
    #{poke struct TT_OS2_, usFirstCharIndex       } ptr $ osUsFirstCharIndex        val
    #{poke struct TT_OS2_, usLastCharIndex        } ptr $ osUsLastCharIndex         val
    #{poke struct TT_OS2_, sTypoAscender          } ptr $ osSTypoAscender           val
    #{poke struct TT_OS2_, sTypoDescender         } ptr $ osSTypoDescender          val
    #{poke struct TT_OS2_, sTypoLineGap           } ptr $ osSTypoLineGap            val
    #{poke struct TT_OS2_, usWinAscent            } ptr $ osUsWinAscent             val
    #{poke struct TT_OS2_, usWinDescent           } ptr $ osUsWinDescent            val
    #{poke struct TT_OS2_, ulCodePageRange1       } ptr $ osUlCodePageRange1        val
    #{poke struct TT_OS2_, ulCodePageRange2       } ptr $ osUlCodePageRange2        val
    #{poke struct TT_OS2_, sxHeight               } ptr $ osSxHeight                val
    #{poke struct TT_OS2_, sCapHeight             } ptr $ osSCapHeight              val
    #{poke struct TT_OS2_, usDefaultChar          } ptr $ osUsDefaultChar           val
    #{poke struct TT_OS2_, usBreakChar            } ptr $ osUsBreakChar             val
    #{poke struct TT_OS2_, usMaxContext           } ptr $ osUsMaxContext            val
    #{poke struct TT_OS2_, usLowerOpticalPointSize} ptr $ osUsLowerOpticalPointSize val
    #{poke struct TT_OS2_, usUpperOpticalPointSize} ptr $ osUsUpperOpticalPointSize val



data TT_Postscript = TT_Postscript
                       { p_FormatType        :: FT_Fixed
...
instance Storable TT_Postscript where
  ...
  poke ptr val = do
    #{poke struct TT_Postscript_, FormatType        } ptr $ p_FormatType        val
    #{poke struct TT_Postscript_, italicAngle       } ptr $ pItalicAngle        val
    #{poke struct TT_Postscript_, underlinePosition } ptr $ pUnderlinePosition  val
    #{poke struct TT_Postscript_, underlineThickness} ptr $ pUnderlineThickness val
    #{poke struct TT_Postscript_, isFixedPitch      } ptr $ pIsFixedPitch       val
    #{poke struct TT_Postscript_, minMemType42      } ptr $ pMinMemType42       val
    #{poke struct TT_Postscript_, maxMemType42      } ptr $ pMaxMemType42       val
    #{poke struct TT_Postscript_, minMemType1       } ptr $ pMinMemType1        val
    #{poke struct TT_Postscript_, maxMemType1       } ptr $ pMaxMemType1        val



data TT_PCLT = TT_PCLT
                 { pclt_Version             :: FT_Fixed
                 , pclt_FontNumber          :: FT_ULong
                 , pclt_Pitch               :: FT_UShort
                 , pcltXHeight              :: FT_UShort
                 , pclt_Style               :: FT_UShort
                 , pclt_TypeFamily          :: FT_UShort
                 , pclt_CapHeight           :: FT_UShort
                 , pclt_SymbolSet           :: FT_UShort
                 , pclt_TypeFace            :: Ptr FT_Char
                 , pclt_CharacterComplement :: Ptr FT_Char
                 , pclt_FileName            :: Ptr FT_Char
                 , pclt_StrokeWeight        :: FT_Char
                 , pclt_WidthType           :: FT_Char
                 , pclt_SerifStyle          :: FT_Byte
                 , pclt_Reserved            :: FT_Byte
                 }
...
instance Storable TT_PCLT where
  ...
  poke ptr val = do
    #{poke struct TT_PCLT_, Version            } ptr $ pclt_Version             val
    #{poke struct TT_PCLT_, FontNumber         } ptr $ pclt_FontNumber          val
    #{poke struct TT_PCLT_, Pitch              } ptr $ pclt_Pitch               val
    #{poke struct TT_PCLT_, xHeight            } ptr $ pcltXHeight              val
    #{poke struct TT_PCLT_, Style              } ptr $ pclt_Style               val
    #{poke struct TT_PCLT_, TypeFamily         } ptr $ pclt_TypeFamily          val
    #{poke struct TT_PCLT_, CapHeight          } ptr $ pclt_CapHeight           val
    #{poke struct TT_PCLT_, SymbolSet          } ptr $ pclt_SymbolSet           val
    #{poke struct TT_PCLT_, TypeFace           } ptr $ pclt_TypeFace            val
    #{poke struct TT_PCLT_, CharacterComplement} ptr $ pclt_CharacterComplement val
    #{poke struct TT_PCLT_, FileName           } ptr $ pclt_FileName            val
    #{poke struct TT_PCLT_, StrokeWeight       } ptr $ pclt_StrokeWeight        val
    #{poke struct TT_PCLT_, WidthType          } ptr $ pclt_WidthType           val
    #{poke struct TT_PCLT_, SerifStyle         } ptr $ pclt_SerifStyle          val
    #{poke struct TT_PCLT_, Reserved           } ptr $ pclt_Reserved            val
...
instance Storable TT_MaxProfile where
  ...
  poke ptr val = do
    #{poke struct TT_MaxProfile_, version              } ptr $ mpVersion               val
    #{poke struct TT_MaxProfile_, numGlyphs            } ptr $ mpNumGlyphs             val
    #{poke struct TT_MaxProfile_, maxPoints            } ptr $ mpMaxPoints             val
    #{poke struct TT_MaxProfile_, maxContours          } ptr $ mpMaxContours           val
    #{poke struct TT_MaxProfile_, maxCompositePoints   } ptr $ mpMaxCompositePoints    val
    #{poke struct TT_MaxProfile_, maxCompositeContours } ptr $ mpMaxCompositeContours  val
    #{poke struct TT_MaxProfile_, maxZones             } ptr $ mpMaxZones              val
    #{poke struct TT_MaxProfile_, maxTwilightPoints    } ptr $ mpMaxTwilightPoints     val
    #{poke struct TT_MaxProfile_, maxStorage           } ptr $ mpMaxStorage            val
    #{poke struct TT_MaxProfile_, maxFunctionDefs      } ptr $ mpMaxFunctionDefs       val
    #{poke struct TT_MaxProfile_, maxInstructionDefs   } ptr $ mpMaxInstructionDefs    val
    #{poke struct TT_MaxProfile_, maxStackElements     } ptr $ mpMaxStackElements      val
    #{poke struct TT_MaxProfile_, maxSizeOfInstructions} ptr $ mpMaxSizeOfInstructions val
    #{poke struct TT_MaxProfile_, maxComponentElements } ptr $ mpMaxComponentElements  val
    #{poke struct TT_MaxProfile_, maxComponentDepth    } ptr $ mpMaxComponentDepth     val



type FT_Sfnt_Tag = FT_Int32
